...
def check_hist():
    ...
    for o in get_data():
        ...
        c.update({ob.start_stage.time + "--" + ob.end_stage.time})
    ...
    width = 0.35
    ax.bar(ind, values, width, color=BAR_COLOR)
    ax.set_xticks(ind)
    ax.set_ylabel("Number of entries present")
    ax.set_title("Expression data of genes across various super structures\n("
                 "as per ZFIN "
                 "classification)")
...
def check_exp_period(gene, organ):
    c = Counter()
    for o in get_data():
        ob = o  # type: DataObject
        if ob.gene_symbol == gene.lower() and ob.super_structure.name == \
                organ.lower():
            c.update({ob.start_stage.time + "--" + ob.end_stage.time})
    ...
    ax.set_xticks(ind)
    ax.set_ylabel("Number of entries present")
    ax.set_title("Expression data of %s in %s\n("
                 "as per ZFIN classification)" % (gene, organ))
    ax.set_xticklabels(labels)
...
if __name__ == "__main__":
    expression_profile("foxc1a")
    # check_exp_period("tdgf1", "heart tube")
    # check_hist()
    # genes_in_organ("heart tube")
    # localization("nkx2.5", False)
